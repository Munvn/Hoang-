...
class AuthController{
        ...
        if(isset($_FILES['image'])){
           ...
           if(move_uploaded_file($tmp_name,"images/".$new_img_name)){
               ...
               $insert_query = mysqli_query($this->conn->connect(),
                   "INSERT INTO nhanvien (unique_id, TenNV, HoNV, email, MatKhau, img, TrangThaiHoatDong)
                         VALUES ({$ran_id}, '${fname}', '${lname}', '${email}', '${encrypt_pass}', '${new_img_name}', '${status}')");
               
               if(!$insert_query){
                echo "Có lỗi chèn xảy ra. Vui lòng thử lại!<br>";
                
                echo "Dữ liệu chèn:<br>";
                echo "unique_id: {$ran_id}<br>";
                echo "TenKH: {$fname}<br>";
                echo "HoKH: {$lname}<br>";
                echo "Email: {$email}<br>";
                echo "Mật khẩu: {$encrypt_pass}<br>";
                echo "Ảnh: {$new_img_name}<br>";
                echo "Trạng thái: {$status}<br>";
                return;
            }
            

               $select_sql2 = mysqli_query($this->conn->connect(), "SELECT * FROM nhanvien WHERE email = '${email}'");
               ...
               $_SESSION['unique_id'] = "1111111111"; // 10 số 1
               echo "success";
           }
        }
    }
    ...
    public function logIn(){
        ...
        $sql = mysqli_query($this->conn->connect(), "SELECT * FROM nhanvien WHERE email = '${email}'");
        ...
        $sql2 = mysqli_query($this->conn->connect(),
            "UPDATE nhanvien SET TrangThaiHoatDong = '{$status}' WHERE unique_id = {$row['unique_id']}");
        if($sql2){
    ...
    public function logOut(){
        ...
        if(isset($logout_id)){
            ...
            $sql = mysqli_query($this->conn->connect(),
                "UPDATE nhanvien SET TrangThaiHoatDong = '{$status}' WHERE unique_id={$logout_id}");
            if($sql){
                session_unset();
                session_destroy();
                header("location: ../login.php");
